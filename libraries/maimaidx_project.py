...
async def music_play_data(payload: dict, songs: str) -> Union[str, dict, MessageSegment, None]:
    payload['version'] = list(set(version for version in plate_to_version.values()))
    data = await get_player_data('plate', payload)
    if isinstance(data, str):
        return data
    ...
    if not player_data:
        ...
        return {"sss":False,"msg":'没玩过, 给我去玩' + game_names[0]}
...
async def music_play_data_dev(payload: dict, songs: str) -> Union[str, MessageSegment, None]:
    ...
    sy.draw(450, 1180, 20, f'Designed by Yuri-YuzuChaN | Generated by {BOTNAME}', (159, 81, 220, 255), 'mm', 2, (255, 255, 255, 255))
    msg = MessageSegment.image(image_to_base64(im))
...
def name_linked(qqid:str):
    ...
    if str(qqid) in name_list:
        return {"success": True, "username": name_list[str(qqid)]['id']}
    else:
