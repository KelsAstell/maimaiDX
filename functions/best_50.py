...
from functions.api import download_music_pictrue
...
class DrawBest:
    ...
    async def whiledraw(self, data: List[ChartInfo], type: bool):
        ...
        for num, info in enumerate(data):
            ...
            if info.fc:
                fc = Image.open(os.path.join(self.maimai_dir, f'UI_MSS_MBase_Icon_{self.fcl[info.fc]}.png')).resize(
                    (45, 45))
                self._im.alpha_composite(fc, (x + 260, y + 98))
            if info.fs:
                fs = Image.open(os.path.join(self.maimai_dir, f'UI_MSS_MBase_Icon_{self.fsl[info.fs]}.png')).resize(
                    (45, 45))
    ...
    async def draw(self):
        ...
        self.dxstar = [Image.open(os.path.join(self.maimai_dir, f'UI_RSL_DXScore_Star_0{_ + 1}.png')).resize((20, 20))
                       for _ in range(3)]
        self.rank = {'d': 'D', 'c': 'C', 'b': 'B', 'bb': 'BB', 'bbb': 'BBB', 'a': 'A', 'aa': 'AA', 'aaa': 'AAA',
        ...
        if self.userName == 'Emo Bot':
            self._meiryo.draw(847, 300, 22, 'Emo Bot Simulation Data', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
        else:
            self._meiryo.draw(847, 300, 22, 'UNiVERSE PLUS Rating System', (0, 0, 0, 255), 'mm', 3,
                              (255, 255, 255, 255))
        self._meiryo.draw(900, 2365, 35, f'Modified by Kels_Astell | Generated by {BOTNAME}',
