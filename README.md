...
**2023-06-21**
1. 修复BUGs
2. 在ra PK时, 如果只填一个用户则默认和自己比较, 可以艾特一位用户进行紧张刺激的仇人对战了.
3. 其他通用BUG修复

**2023-06-19**
1. 导到嗦梨了, 所以加了个新的命令: 嗦梨进度/嗦🍐进度/🍐进度
   1. 该命令将显示你的嗦梨统计信息
2. 修复历史遗留问题及代码中不规范类型/未异步处理的问题
3. 分离本地别名库插件为一个新插件.
...
| 嗦梨进度/嗦🍐进度/🍐进度[@someone]                          | 查看你或别人的嗦梨毛到了多少分                                                          |
| rapk/ra比较/仇人对战 <名字1> <名字2>                         | 查看两个用户的ra比较结果, 可以用@指定一名用户, 需要本地缓存.                                       |
